...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admins.cities.index') }}">{{ __('dashboard.term-index') }} </a>
            </li>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-end mb-5">
                        <a href="{{ route('admins.terms.create') }}" class="btn-sm btn-primary btn-icon-text"> <i
                                class="fa-solid fa-plus"></i> Add New term
                        </a>
                    </div>
                            ...
                            <tbody>
                                @foreach ($terms as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if (empty($item->getTranslation('title', 'en')))
                                                Not Have Title
                                            @else
                                        ...
                                        <td>
                                            @if (empty($item->getTranslation('title', 'ar')))
                                                ليس لديه عنوان
                                            @else
                                                {{ @$item->getTranslation('title', 'ar') }}
                                            @endif
                                        </td>
                                        <td>
                                            <div class="form-check form-switch mb-2" id="status">
                                                <form method="get" action="{{ route('admins.terms.status', $item->id) }}"
                                                    class="d-inline">
                                                    @csrf
                                        ...
                                        <td>
                                            <a href="{{ route('admins.terms.show', $item->id) }}"
                                                class="btn-sm btn-secondary btn-icon-text"> <i
                                                    class="fa-solid fa-eye p-1"></i>view
                                            </a>

                                            <a href="{{ route('admins.terms.edit', $item->id) }}"
                                                class="btn-sm btn-primary btn-icon-text m-1 "><i
                                                    class="fa-solid fa-pen-to-square"></i> edit
                                            </a>


                                            <form method="POST" action="{{ route('admins.terms.destroy', $item->id) }}"
                                                class="d-inline">
                                                @csrf
                                                @method('delete')
