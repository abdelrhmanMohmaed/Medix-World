...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="{{route ('admins.cities.index') }}">{{ __('dashboard.booking-index')}} </a></li>
    <li class="breadcrumb-item active" aria-current="page">{{ __('dashboard.booking-index')}} </li>
  </ol>
</nav>
          ...
          <table id="dataTableExample" class="table">
            ...
            <thead>
              <tr>
                ...
                <th>{{ __('dashboard.title-patient') }}</th>
                <th>{{ __('dashboard.title-service-provider') }}</th>
              ...
              <tr>
                ...
                <td>
                  ...
                  <form method="POST" action="{{ route('admins.advices.destroy', $item->id) }}" class="d-inline"> 
                    @csrf
                    @method('delete')
                    <button type="submit" class="btn btn-danger btn-xs btn-icon-text p-1" onclick="return confirm('Are you sure to delete this advice')"><i class="btn-icon-prepend fa-solid fa-trash"></i>Delete</button>
                  </form> -->
